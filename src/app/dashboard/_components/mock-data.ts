...
export const MOCK_TASKS: MockTask[] = [
  ...
  { id: "t2", title: "Review type scale", project_id: "p3", status: "todo", priority: 2, due_date: fmt(today), start_date: fmt(today), estimated_hours: 1, description: null, created_by: "u1" },
  { id: "t3", title: "Push notification flow", project_id: "p2", status: "in_progress", priority: 1, due_date: fmt(today), start_date: fmt(addDays(today, -1)), estimated_hours: 4, description: "Map permission request → notification → deep link", created_by: "u1" },
  ...
  { id: "t5", title: "Onboarding screens", project_id: "p2", status: "todo", priority: 3, due_date: fmt(addDays(today, 3)), start_date: fmt(addDays(today, 2)), estimated_hours: 6, description: null, created_by: "u1" },
...
export const PROJECT_COLORS: Record<string, { tag: string; dim: string }> = {
  p1: { tag: "#E8C547", dim: "#4A4020" },  // Gold
  p2: { tag: "#47E8B5", dim: "#204A3A" },  // Green
  p3: { tag: "#E84747", dim: "#4A2020" },  // Red
};
